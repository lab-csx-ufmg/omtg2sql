-- Validate the planar subdivision on cidade
CREATE OR REPLACE FUNCTION val_pla_sub_cid
RETURN VARCHAR IS
...
BEGIN
	 ...
	 FOR p IN (SELECT ps1.rowid as rowid1, ps2.rowid as rowid2
	    ...
	    INSERT INTO spatial_error (type, error) 
           VALUES ('Planar Subdivision Error', 'Spatial relation between cidade'||p1_columns||' and'||p2_columns||' is not touch or disjoint');
...
END;
/
-- Validate the spatial aggregation between the whole cidade and the part bairro
CREATE OR REPLACE FUNCTION val_spa_agr_cid_bai
RETURN VARCHAR IS
...
BEGIN
	 ...
	 FOR w IN (SELECT distinct w_rowid FROM sa_aux) LOOP
	 	 ...
	 	 FOR p IN (SELECT sa.p_rowid
	 	 	...
	 	 	INSERT INTO spatial_error (type, error)
		       VALUES ('Spatial Aggregation Error', 'bairro'||p_columns||' intersection cidade'||w_columns|| ' is not equal to part');
			p_contains_error := TRUE;
			COMMIT;
	 ...
	 FOR w IN (SELECT distinct w_rowid FROM sa_aux) LOOP
	 	 ...
	 	 FOR p IN (SELECT sa1.p_rowid as p_rowid1, sa2.p_rowid as p_rowid2
	 	 	...
	 	 	INSERT INTO spatial_error (type, error)
		       VALUES ('Spatial Aggregation Error', 'Spatial relation between parts bairro'||p_columns||' and'||w_columns||' is not touch or disjoint');
			p_contains_error := TRUE;
			COMMIT;
		 ...
		 END LOOP;
	 	 ...
	 	 FOR ww IN (SELECT * 
			...
			SELECT ' idCidade = '||idCidade||' nomeCidade = '||nomeCidade INTO w_columns
	 	 	...
	 	 	INSERT INTO spatial_error (type, error)
		       VALUES ('Spatial Aggregation Error', 'cidade'||w_columns||' intersection all its parts is not equal to whole');
			p_contains_error := TRUE;
			COMMIT;
