...
public class OffLineConstraintsWriter extends SQLWriter {
	...
	private final String VALIDATE_SPATIAL_AGGREGATION_TEMPLATE = 
		"validation/validate_spatial_aggregation_template.sql";
	private final String VALIDATE_PLANAR_SUBDIVISION_TEMPLATE = 
		"validation/validate_planar_subdivision_template.sql";
	private final String VALIDATE_ISOLINE_TEMPLATE = 
		"validation/validate_isoline_template.sql";
	private final String VALIDATE_NETWORK_TEMPLATE = 
		"validation/validate_arc_node_network_template.sql";
	private final String VALIDATE_TIN_TEMPLATE = 
		"validation/validate_tin_template.sql";

	private final String AUX_GET_POINT_FUNCTION = "validation/aux_get_point.sql";
	private final String AUX_JOIN_GEOMETRY_FUNCTION = "validation/aux_join_geometry.sql";

	private Scanner in;
	private boolean create_aux_get_point_function, create_aux_join_geometry_function;
...
//	}
	...
	private String processSpatialAggegationConstraint(String sql, String wholeTableName, 
			List<String> wholeTableKeys, String partTableName, List<String> partTableKeys) {

		if (sql.contains(COMMENT)) {

			sql = FormatSQL.replaceAll(sql, COMMENT, "-- Validate the spatial aggregation between the whole " 
					+ wholeTableName + " and the part " + partTableName);
		} 

		if (sql.contains(VAL_SPA_AGR_NAME)) {

			sql = FormatSQL.replaceAll(sql, VAL_SPA_AGR_NAME, FormatSQL.validateLengthName(wholeTableName) + "_" + 
	...
	private String processPlanarSubdivisionConstraint(String sql, 
		...
		if (sql.contains(COMMENT)) {
			...
			sql = FormatSQL.replaceAll(sql, COMMENT, "-- Validate the planar subdivision on " 
					+ planarSubTableName);
		} 

		if (sql.contains(VAL_PLA_SUB_NAME)) {

			sql = FormatSQL.replaceAll(sql, VAL_PLA_SUB_NAME, FormatSQL.validateLengthName(planarSubTableName));
		} 

		if (sql.contains(PLANAR_SUB_TABLE_NAME)) {

			sql = FormatSQL.replaceAll(sql, PLANAR_SUB_TABLE_NAME, planarSubTableName);
		}
	...
	private String processNetworkConstraint(String sql, String arcTableName, 
			List<String> arcTableKeys, String nodeTableName, List<String> nodeTableKeys) {

		if (sql.contains(COMMENT)) {

			sql = FormatSQL.replaceAll(sql, COMMENT, "-- Validate the network between the " 
					+ arcTableName + " and " + nodeTableName);
		} 

		if (sql.contains(VAL_NETWOK_NAME)) {

			sql = FormatSQL.replaceAll(sql, VAL_NETWOK_NAME, FormatSQL.validateLengthName(arcTableName) + "_" + 
	...
	private String processIsolineConstraint(String sql, 
		...
		if (sql.contains(COMMENT)) {
			...
			sql = FormatSQL.replaceAll(sql, COMMENT, "-- Validate the isoline on " 
					+ isolineTableName);
		} 

		if (sql.contains(VAL_ISOLINE_NAME)) {

			sql = FormatSQL.replaceAll(sql, VAL_ISOLINE_NAME, FormatSQL.validateLengthName(isolineTableName));
		} 

		if (sql.contains(ISOLINE_TABLE_NAME)) {

			sql = FormatSQL.replaceAll(sql, ISOLINE_TABLE_NAME, isolineTableName);
		}
	...
	private String processTINConstraint(String sql, String TINTableName, List<String> TINTableKeys) {
		...
		if (sql.contains(VAL_TIN_NAME)) {
			...
			sql = FormatSQL.replaceAll(sql, VAL_TIN_NAME, FormatSQL.validateLengthName(TINTableName));
		} 
		...
		if (sql.contains(TIN_TABLE_KEYS)) {
			...
			sql = FormatSQL.replaceAll(sql, TIN_TABLE_KEYS, FormatSQL.columnsToString(TINTableKeys));
		}
	...
	public void appendTINConstraint(String TINTableName, List<String> TINTableKeys) {
	...
	public void appendSpatialAggregationConstraint(String wholeTableName, List<String> wholeTableKeys, 
		...
		while (in.hasNextLine()) {
			...
			String sql = in.nextLine();
			sql = processSpatialAggegationConstraint(sql, wholeTableName, wholeTableKeys, 
					partTableName, partTableKeys);
			appendSQL(sql);
		}
	...
	public void appendNetworkConstraint(String arcTableName, List<String> arcTableKeys,
			String nodeTableName, List<String> nodeTableKeys) {
